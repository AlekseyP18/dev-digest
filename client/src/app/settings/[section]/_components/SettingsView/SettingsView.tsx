...
export function SettingsView({ section = DEFAULT_SECTION }: { section?: string }) {
  ...
  const label = t(`sections.${current.key}`);

  return (
    <AppShell crumb={[{ label: t("breadcrumb"), href: "/settings/api-keys" }, { label }]}>
      <div style={s.layout}>
        <div style={s.nav}>
          ...
          {SETTINGS_SECTIONS.map((sec) => {
            ...
            return (
              <Link key={sec.key} href={`/settings/${sec.key}`} aria-current={on ? "page" : undefined} style={s.navItem(on)}>
                {t(`sections.${sec.key}`)}
              </Link>
            );
          })}
        ...
        <div style={s.pane}>
          {section === SECTION_API_KEYS ? (
            ...
            <EmptyState icon="Settings" title={label} body={t("fallbackBody", { label })} />
          )}
        </div>
      </div>
    </AppShell>
