...
describe("PRRow — navigation", () => {
  it("the title is a real link to the PR detail (keyboard, middle-click, new tab)", () => {
    renderRow(pr({}));
    expect(screen.getByRole("link", { name: "Add rate limiting to public API endpoints" })).toHaveAttribute(
      "href",
      "/repos/repo-1/pulls/482",
    );
  });
});
...
describe("PRRow — findings column", () => {
  ...
  it("hover lazily loads the latest review of each agent, read-only", () => {
    ...
    // The popover is portaled outside the row link, so a click inside it can't navigate.
    expect(within(dialog).getByText("Hardcoded Stripe secret key in commit").closest("a")).toBeNull();
  });
