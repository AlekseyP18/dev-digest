...
/** Constants for the PR list screen (/repos/:repoId/pulls). */
...
export const OPEN_STATUSES: ReadonlySet<PrStatus> = new Set(["needs_review", "reviewed", "stale"]);

/** Status filter when `?status=` is absent — the most actionable view. */
export const DEFAULT_STATUS_FILTER = "needs_review";

export const SORT_KEYS = ["newest", "oldest"] as const;
export type SortKey = (typeof SORT_KEYS)[number];

/** Debounce (ms) before the search box is written to `?q=`. */
export const QUERY_URL_DEBOUNCE_MS = 300;
