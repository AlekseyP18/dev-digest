...
import { toForm, toPatch, type AgentForm } from "./helpers";
import { s } from "./styles";

/**
 * Config tab — name/description/provider/model/system-prompt + enabled toggle.
 * The form is initialised from `agent` once; the parent remounts it per agent
 * (`key={agent.id}`), so switching agents starts from that agent's values.
 */
export function ConfigTab({ agent }: { agent: Agent }) {
  const t = useTranslations("agents");
  const toast = useToast();
  const update = useUpdateAgent();
  const [form, setForm] = React.useState<AgentForm>(() => toForm(agent));
  const set = <K extends keyof AgentForm>(key: K) => (value: AgentForm[K]) =>
    setForm((prev) => ({ ...prev, [key]: value }));

  const { data: models } = useProviderModels(form.provider);
  ...
  const modelOptions = withCurrentOption(toModelOptions(models), form.model);
  ...
  const outputSchemaOptions = [{ value: OUTPUT_SCHEMA_VALUE, label: t("config.outputSchemaOption") }];

  const save = () =>
    update.mutate(
      { id: agent.id, patch: toPatch(form) },
      {
  ...
  return (
    <div style={s.wrap}>
      ...
      <FormField
        ...
        hint={noModels ? t("config.modelEmptyHint", { provider: form.provider }) : t("config.modelHint")}
      >
        <SearchableSelect
          value={form.model}
          onChange={set("model")}
      ...
      <FormField label={t("config.outputSchema")}>
        <SelectInput value={OUTPUT_SCHEMA_VALUE} options={outputSchemaOptions} />
      </FormField>
      <div style={s.actions}>
