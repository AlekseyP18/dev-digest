...
## Map
- `src/app/**/page.tsx` thin server entries (params, metadata, Suspense) → client `_components/<Name>View/`; feature logic colocated in `_components/<Name>/` (+ `*.test.tsx`)
- `src/lib/api.ts` single fetch client · `src/lib/hooks/<domain>.ts` every data hook · `src/lib/hooks/keys.ts` every query key
- `src/lib/<noun>.ts` pure rules shared by screens (severity, run outcome, dates, cost)
...
- Every new `_components/<Name>/` ships with its own `*.test.tsx` (render via `renderWithIntl` from `@/test/intl`).
- Import boundaries are lint-enforced: `components/` + `lib/` never import `app/`; hooks are imported from their domain file (no barrel).
